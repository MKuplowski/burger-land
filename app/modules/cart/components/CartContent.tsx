    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gray-50">
        <div className="text-center space-y-4">
          <h1 className="text-2xl font-bold text-gray-900">{t("sections.cart.empty")}</h1>
          <div className="mt-2">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{t("sections.cart.title")}</h1>
          <Button variant="secondary" asChild>
            <Link href="/">{t("actions.continueShopping")}</Link>
          </Button>
        </div>
          ...
          footer={
            <div className="w-full flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="text-xl font-bold text-gray-900">
                {t("sections.cart.total")}: $<span data-testid={TEST_IDS.TOTAL_PRICE_VALUE}>{cart.total.toFixed(2)}</span>
              </div>
